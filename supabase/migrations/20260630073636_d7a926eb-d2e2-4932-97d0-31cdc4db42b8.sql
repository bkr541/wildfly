...
ALTER TABLE public.plans ALTER COLUMN entitlement_tier SET DEFAULT 'free';
UPDATE public.plans SET entitlement_tier = 'free' WHERE entitlement_tier IS NULL;
ALTER TABLE public.plans ALTER COLUMN entitlement_tier SET NOT NULL;
ALTER TABLE public.plans DROP CONSTRAINT IF EXISTS plans_entitlement_tier_check;
ALTER TABLE public.plans ADD CONSTRAINT plans_entitlement_tier_check
...
GRANT SELECT ON public.user_search_usage_monthly TO authenticated;
GRANT ALL ON public.user_search_usage_monthly TO service_role;
...
GRANT SELECT ON public.search_usage_events TO authenticated;
GRANT ALL ON public.search_usage_events TO service_role;
...
DROP POLICY IF EXISTS "Users can view own monthly search usage" ON public.user_search_usage_monthly;
CREATE POLICY "Users can view own monthly search usage"
  ON public.user_search_usage_monthly FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own search usage events" ON public.search_usage_events;
CREATE POLICY "Users can view own search usage events"
  ON public.search_usage_events FOR SELECT USING (auth.uid() = user_id);

CREATE OR REPLACE FUNCTION public.resolve_search_entitlement(p_user_id uuid)
RETURNS TABLE (plan_id text, plan_name text, entitlement_tier text, monthly_limit integer)
...
AS $$
  ...
  FROM (SELECT 1) seed
  LEFT JOIN LATERAL (
    SELECT p.* FROM public.user_subscriptions us
  ...
  LEFT JOIN LATERAL (
    SELECT p.* FROM public.plans p WHERE p.id = 'free' LIMIT 1
  ) free_plan ON true;
$$;
...
BEGIN
  ...
  SELECT used_count INTO v_used FROM public.user_search_usage_monthly
  WHERE user_id = v_uid AND period_start = v_period_start FOR UPDATE;

  SELECT * INTO v_existing FROM public.search_usage_events
  WHERE user_id = v_uid AND request_id = p_request_id FOR UPDATE;

  IF FOUND AND v_existing.status = 'authorized' THEN
    RETURN jsonb_build_object(
      'allowed', true, 'reason', 'ALREADY_PROCESSED', 'already_processed', true,
      'request_id', p_request_id, 'plan_id', v_plan_id, 'plan_name', v_plan_name,
      'tier', v_tier, 'limit', v_limit, 'used', v_used,
      'remaining', CASE WHEN v_tier = 'paid' THEN NULL ELSE GREATEST(0, v_limit - v_used) END,
      'period_start', v_period_start, 'period_end', v_period_end, 'charged', 0
    );
  END IF;
  ...
  IF v_tier = 'free' AND v_used >= v_limit THEN
    ...
    RETURN jsonb_build_object(
      'allowed', false, 'reason', 'MONTHLY_LIMIT_REACHED', 'already_processed', false,
      'request_id', p_request_id, 'plan_id', v_plan_id, 'plan_name', v_plan_name,
  ...
  RETURN jsonb_build_object(
    'allowed', true, 'reason', NULL, 'already_processed', false,
    'request_id', p_request_id, 'plan_id', v_plan_id, 'plan_name', v_plan_name,
    'tier', v_tier, 'limit', v_limit, 'used', v_used,
    'remaining', CASE WHEN v_tier = 'paid' THEN NULL ELSE GREATEST(0, v_limit - v_used) END,
...
CREATE OR REPLACE FUNCTION public.refund_authorized_search(
  p_user_id uuid, p_request_id uuid, p_reason text DEFAULT 'UPSTREAM_FAILURE'
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_event FROM public.search_usage_events
  ...
  IF v_event.status = 'refunded' THEN RETURN jsonb_build_object('refunded', false, 'reason', 'ALREADY_REFUNDED'); END IF;
  IF v_event.status <> 'authorized' THEN RETURN jsonb_build_object('refunded', false, 'reason', 'NOT_AUTHORIZED'); END IF;
  ...
  SET status = 'refunded',
      ...
      updated_at = now()
  WHERE id = v_event.id;
  RETURN jsonb_build_object('refunded', true, 'request_id', p_request_id, 'used', v_used);
END;
$$;

REVOKE ALL ON FUNCTION public.refund_authorized_search(uuid, uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.refund_authorized_search(uuid, uuid, text) TO service_role;
...
DECLARE
  v_uid uuid := auth.uid();
  v_plan_id text; v_plan_name text; v_tier text; v_limit integer;
  v_period_start date := date_trunc('month', timezone('UTC', now()))::date;
  v_period_end date := (date_trunc('month', timezone('UTC', now())) + interval '1 month')::date;
  v_used integer := 0;
BEGIN
  ...
  v_tier := COALESCE(v_tier, 'free');
  v_limit := CASE WHEN v_tier = 'paid' THEN NULL ELSE COALESCE(v_limit, 5) END;
  SELECT COALESCE(used_count, 0) INTO v_used FROM public.user_search_usage_monthly
  WHERE user_id = v_uid AND period_start = v_period_start;
  RETURN jsonb_build_object(
    'authenticated', true, 'plan_id', v_plan_id, 'plan_name', v_plan_name,
    'tier', v_tier, 'is_paid', v_tier = 'paid', 'limit', v_limit,
...
CREATE OR REPLACE FUNCTION public.consume_search_credits(
  p_trip_type text, p_arrival_airports_count integer,
  p_all_destinations boolean, p_source_id text DEFAULT NULL
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
REVOKE ALL ON FUNCTION public.consume_search_credits(text, integer, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.consume_search_credits(text, integer, boolean, text) TO authenticated;
