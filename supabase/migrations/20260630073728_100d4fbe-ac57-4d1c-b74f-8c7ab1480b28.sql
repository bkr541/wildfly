CREATE INDEX IF NOT EXISTS idx_flight_searches_scheduled_origin_date_latest
  ON public.flight_searches (
    ...
    provider_observed_at DESC, search_timestamp DESC
  )
...
CREATE INDEX IF NOT EXISTS idx_flight_snapshots_available_by_search
  ON public.flight_snapshots (flight_search_id, departure_at, stable_itinerary_key)
  WHERE has_go_wild = true AND availability_status = 'returned';
...
DECLARE
  ...
  v_home_airport text; v_airport_name text;
  v_home_city text; v_home_state text;
  v_timezone text; v_timezone_group text;
  v_local_date date; v_search_id uuid;
  v_observed_at timestamptz; v_job_status text;
  v_flights jsonb := '[]'::jsonb; v_status text;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;

  SELECT upper(trim(ui.home_airport)) INTO v_home_airport
  FROM public.user_info ui WHERE ui.auth_user_id = v_user_id LIMIT 1;

  IF coalesce(v_home_airport, '') = '' THEN
    RETURN jsonb_build_object('status', 'home_airport_missing', 'flights', '[]'::jsonb);
  END IF;

  SELECT a.name, coalesce(l.city, l.name), l.state_code, a.timezone
  INTO v_airport_name, v_home_city, v_home_state, v_timezone
  FROM public.airports a
  LEFT JOIN public.locations l ON l.id = a.location_id
  WHERE upper(a.iata_code) = v_home_airport AND a.is_active = true
  LIMIT 1;

  IF coalesce(v_timezone, '') = '' THEN
    RETURN jsonb_build_object(
      'status', 'timezone_missing', 'homeAirport', v_home_airport,
      'airportName', v_airport_name, 'flights', '[]'::jsonb
    );
  END IF;
  ...
  SELECT fs.id, coalesce(fs.provider_observed_at, fs.search_timestamp)
  ...
  IF v_timezone_group IS NOT NULL THEN
    SELECT b.status INTO v_job_status
    FROM public.bulk_search_job_logs b
    WHERE b.timezone_group = v_timezone_group AND b.target_date = v_local_date
  ...
  IF v_search_id IS NULL THEN
    ...
    END;
    RETURN jsonb_build_object(
      'status', v_status, 'homeAirport', v_home_airport,
      'airportName', v_airport_name, 'homeCity', v_home_city,
      'homeState', v_home_state, 'homeAirportTimezone', v_timezone,
      'localDate', v_local_date, 'jobStatus', v_job_status,
      'flights', '[]'::jsonb
    );
  END IF;
  ...
  WITH ranked AS (
    ...
    SELECT r.*, da.timezone AS destination_timezone,
           coalesce(dl.city, dl.name) AS destination_city,
           dl.state_code AS destination_state
    FROM ranked r
    LEFT JOIN public.airports da ON upper(da.iata_code) = upper(r.leg_destination_iata)
  ...
  SELECT coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'airline', a.airline, 'flightNumber', a.flight_number,
        ...
        'flightType', a.flight_type, 'stops', a.stops,
        'duration', a.total_duration_display, 'cabin', a.display_cabin,
        'goWildPrice', a.go_wild_total, 'standardPrice', a.standard_total,
  ...
  INTO v_flights FROM available a;
  ...
  RETURN jsonb_build_object(
    ...
    'airportName', v_airport_name, 'homeCity', v_home_city,
    'homeState', v_home_state, 'homeAirportTimezone', v_timezone,
    'localDate', v_local_date, 'observedAt', v_observed_at,
    'jobStatus', v_job_status, 'flights', v_flights
  );
END;
$$;
...
SELECT users.user_id, 'todays_gowild_flights',
       least(coalesce(max(existing."order"), 0) + 1, 32767)::smallint, 'active'
FROM (
  SELECT DISTINCT auth_user_id AS user_id FROM public.user_info WHERE auth_user_id IS NOT NULL
  UNION
  SELECT DISTINCT user_id FROM public.user_homepage
) users
LEFT JOIN public.user_homepage existing ON existing.user_id = users.user_id
WHERE NOT EXISTS (
  SELECT 1 FROM public.user_homepage check_row
