-- Home GoWild and Day Trips snapshot fixes.
-- 1) Replaces get_next_home_gowild_summary() so route stats use leg_destination_iata,
--    matching the current flight_snapshots schema.
-- 2) Adds get_home_day_trips_from_snapshots(), which builds the Home Day Trips
--    widget from nightly scheduled bulk-search flight_snapshots instead of flight-cache.
