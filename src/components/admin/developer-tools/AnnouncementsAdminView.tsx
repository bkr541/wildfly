...
export function AnnouncementsAdminView() {
  const { announcements, loading, saving, error, reload, create } = useAnnouncementsAdmin();
  ...
  const newButton = (
    ...
    >
      <HugeiconsIcon icon={showAdd ? Cancel01Icon : PlusSignIcon} size={13} color="white" strokeWidth={2} />
      {showAdd ? "Cancel" : "New"}
    </button>
  );
  ...
  return (
    <DeveloperToolsAdminShell
      ...
      loading={loading}
      error={error}
      actions={saving ? <AdminSavingIndicator /> : newButton}
    >
      {/* ── Create form ── */}
      {showAdd && (
        <AdminCard>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="h-10 rounded-xl bg-[#059669] text-white text-sm font-semibold hover:bg-[#047857] transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
            ...
            return (
              <AdminCard key={ann.id} className="p-0 overflow-hidden">
                ...
                <button
                  ...
                  className="w-full text-left px-5 py-4 flex items-start gap-3 hover:bg-[#F9FAFB] transition-colors"
                >
                  <span
                  ...
                  <div className="flex items-center gap-1 shrink-0 bg-[#F3F4F6] px-2 py-1 rounded-full">
                    ...
                    <span className="text-[11px] font-semibold text-[#6B7280]">{ann._view_count}</span>
                  </div>
                </button>

                {/* Expanded detail */}
                {isExpanded && (
                  <div className="border-t border-[#EEF0F0] px-5 py-4 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={label} className="flex items-start gap-1.5">
                          <HugeiconsIcon icon={Clock01Icon} size={11} color="#9CA3AF" strokeWidth={1.5} className="mt-0.5 shrink-0" />
                          <div>
