...
import { listMessages, listDeliveries, retryDeliveries } from "@/services/adminMessaging";
...
import type { MessagingMessage, MessagingRecipient, RecipientStatus } from "./messagingTypes";
...
export function MessagingOverview() {
  ...
  useEffect(() => {
    listMessages({ page: 0, page_size: 100 })
  ...
  const completed = messages.filter(m => m.status === "completed" || m.status === "partially_completed");
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard icon={Mail01Icon} label="Total Messages" value={messages.length} />
