...
const DEFAULT_BODIES: Record<string, string> = {
  "1": JSON.stringify({ targetUrl: `https://gowilder.net/flights/ATL/MIA/${new Date().toISOString().slice(0, 10)}`, origin: "ATL", destination: "MIA" }, null, 2),
  "2": JSON.stringify({ targetUrl: `https://www.flight.info/ATL/departures/${new Date().toISOString().slice(0, 10)}`, origin: "ATL", destination: "MIA" }, null, 2),
      ...
      {rows.map(row => (
        <div key={row.id} className="grid grid-cols-[16px_1fr_1fr_20px] gap-1.5 items-center">
          ...
          <input value={row.value} onChange={e => update(row.id, "value", e.target.value)}
            ...
            className="w-full px-2 py-1.5 rounded-lg border border-[#E3E6E6] text-xs font-mono text-[#2E4A4A] placeholder:text-[#C4CACA] focus:outline-none focus:border-[#345C5A] bg-[#F8F9F9]" />
          <button type="button" onClick={() => remove(row.id)} className="text-[#C4CACA] hover:text-red-400 transition-colors">
  ...
  <div className="flex gap-0 border-b border-[#E3E6E6]">
    {tabs.map(t => (
      <button key={t} type="button" onClick={() => onSelect(t)}
        className={`px-3 py-2 text-xs font-semibold transition-colors border-b-2 -mb-px ${active === t
...
const JsonNode = ({ data, depth = 0 }: { data: unknown; depth?: number }) => {
  ...
  if (entries.length === 0) return <span className="text-[#6B7B7B]">{open}{close}</span>;
  ...
  return (
    <span>
      ...
      {collapsed ? (
        <span className="text-[#6B7B7B]"> …{entries.length} {isArr ? "items" : "keys"} </span>
      ) : (
        <>
          {"\n"}
          {entries.map(([k, v], i) => (
...
const ApiClientScreen = ({ onBack }: ApiClientScreenProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (ct.includes("json")) {
        try { JSON.parse(text); bodyType = "json"; } catch { /* keep text */ }
      }

      setResponse({ status: res.status, statusText: res.statusText, timeMs, sizeBytes, headers: resHeaders, body: text, bodyType });
      setResTab("Body");
    } catch (err: any) {
      const timeMs = Date.now() - start;
      if (err?.name === "AbortError") {
        setResponse({ status: 0, statusText: "Aborted", timeMs, sizeBytes: 0, headers: {}, body: "Request was aborted or timed out.", bodyType: "text" });
      } else {
        setResponse({ status: 0, statusText: "Network Error", timeMs, sizeBytes: 0, headers: {}, body: err?.message ?? "Unknown error", bodyType: "text" });
      }
    } finally {
  ...
  const filteredSaved = saved.filter(r =>
    ...
    r.url.toLowerCase().includes(searchQ.toLowerCase())
  );
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 text-xs">
              ...
              {reqTab === "Body" && (
                <div className="space-y-3">
                  ...
                  {bodyType === "json" && (
                    <div className="relative">
                      <div className="flex gap-1.5 absolute top-2 right-2 z-10">
                        <button type="button" onClick={() => setBodyJson(prettyJson(bodyJson))}
                          className="text-[9px] px-1.5 py-0.5 rounded bg-[#E3E6E6] text-[#6B7B7B] hover:bg-[#D6D9D9] transition-colors">Pretty</button>
                        <button type="button" onClick={() => setBodyJson(minifyJson(bodyJson))}
                  ...
                  {bodyType === "form-data" && (
                    <div className="space-y-3">
                      <KVTable rows={bodyFormData} onChange={setBodyFormData} placeholder="field" valuePlaceholder="value" />
                      <div>
                        <p className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider mb-1">File upload</p>
                        ...
                        {fileInput && <p className="text-[10px] text-[#345C5A] mt-1">Selected: {fileInput.name} ({formatBytes(fileInput.size)})</p>}
                      </div>
                    </div>
                  )}

                  {bodyType === "urlencoded" && (
                    <KVTable rows={bodyUrlEncoded} onChange={setBodyUrlEncoded} placeholder="key" valuePlaceholder="value" />
                  )}
              ...
              {reqTab === "Auth" && (
                <div className="space-y-3">
                  ...
                  {authType === "bearer" && (
                    <div>
                      <label className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider block mb-1">Bearer Token</label>
                      ...
                      <p className="text-[10px] text-[#6B7B7B] mt-1 flex items-center gap-1">
                        <HugeiconsIcon icon={InformationCircleIcon} size={10} color="currentColor" strokeWidth={1.5} />
                  ...
                  {authType === "basic" && (
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider block mb-1">Username</label>
                        <input value={basicUser} onChange={e => setBasicUser(e.target.value)}
                          placeholder="username"
                          className="w-full px-3 py-2 rounded-xl border border-[#E3E6E6] text-xs font-mono text-[#2E4A4A] bg-[#F8F9F9] focus:outline-none focus:border-[#345C5A]" />
                      </div>
                      <div>
                        <label className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider block mb-1">Password</label>
                  ...
                  {authType === "apikey" && (
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider block mb-1">Key name</label>
                          <input value={apiKeyName} onChange={e => setApiKeyName(e.target.value)}
                            className="w-full px-3 py-2 rounded-xl border border-[#E3E6E6] text-xs font-mono text-[#2E4A4A] bg-[#F8F9F9] focus:outline-none focus:border-[#345C5A]" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold text-[#6B7B7B] uppercase tracking-wider block mb-1">Value</label>
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <div className={`h-5 w-9 rounded-full relative transition-colors ${followRedirects ? "bg-[#345C5A]" : "bg-[#D1D5D5]"}`}
                      ...
                      <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${followRedirects ? "translate-x-4" : "translate-x-0.5"}`} />
                    </div>
