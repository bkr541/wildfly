...
import { type ReliableItineraryRoute } from "./itineraryHelpers";
...
interface Props {
  data: ReliableItineraryRoute | null;
    ...
    <div className="rounded-2xl bg-white p-5 flex flex-col" style={{ boxShadow: CARD_SHADOW }}>
      ...
      >
        ...
        <div className="flex-1 ml-2">
          ...
          <p className="text-xs text-[#6B7B7B]">Highest itinerary reliability score in the selected period</p>
        </div>
        ...
        <div className="overflow-hidden">
          {!data ? (
            <p className="text-sm text-[#9CA3AF] text-center py-6">
              Not enough data yet. Reliability requires routes with at least 30 itineraries observed across 7 or more snapshot days.
            </p>
          ) : (
            <>
              <div className="flex items-start justify-center gap-3 mb-4">
                <div className="flex flex-col items-center">
                  ...
                  {originLabel && <span className="text-[11px] text-[#9CA3AF] mt-1">{originLabel}</span>}
                </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  {destLabel && <span className="text-[11px] text-[#9CA3AF] mt-1">{destLabel}</span>}
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div className="bg-green-50 rounded-xl px-3 py-2.5 flex flex-col">
                  <p className="text-[9px] font-semibold text-green-700 uppercase tracking-wider mb-1">Reliability Score</p>
                  <p className="text-xl font-bold text-green-700 leading-none">{data.reliabilityScore.toFixed(1)}</p>
                </div>
                <div className="bg-green-50 rounded-xl px-3 py-2.5 flex flex-col">
                  <p className="text-[9px] font-semibold text-green-700 uppercase tracking-wider mb-1">Avg Availability</p>
                  <p className="text-xl font-bold text-green-700 leading-none">{data.avgAvailabilityRate.toFixed(1)}%</p>
                </div>
                <div className="border border-gray-100 rounded-xl px-3 py-2.5 flex flex-col">
                  <p className="text-[9px] font-semibold text-[#6B7B7B] uppercase tracking-wider mb-1">Itineraries</p>
                  <p className="text-base font-bold text-[#2E4A4A] leading-none">{data.sampleSize}</p>
                  <p className="text-[10px] text-[#9CA3AF] mt-0.5">{data.snapshotDays} snapshot days</p>
                </div>
                <div className="border border-gray-100 rounded-xl px-3 py-2.5 flex flex-col items-center justify-center gap-0.5">
                  ...
                  <p className="text-[10px] text-[#9CA3AF]">daily variance</p>
                </div>
              </div>
            </>
          )}
