...
import { AnalyticsUpIcon, ArrowDown01Icon } from "@hugeicons/core-free-icons";
...
const TopRoutesCard = ({ result, routes }: Props) => {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 flex flex-col" style={{ boxShadow: CARD_SHADOW }}>
      <div className={`flex items-center justify-between cursor-pointer ${isExpanded ? "mb-4" : ""}`}
        ...
        <HugeiconsIcon icon={AnalyticsUpIcon} size={28} color="#059669" strokeWidth={1.5} />
        <div className="flex-1 ml-2">
          <div className="flex items-center gap-2">
            <p className="text-base font-semibold text-[#059669] uppercase tracking-wider">Top 5 Routes</p>
          ...
          <p className="text-xs text-[#6B7B7B]">Highest GoWild itinerary rate (≥{result?.threshold ?? 30} itineraries)</p>
        </div>
              ...
              {list.map((r: any, i: number) => (
                <div key={r.route}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-green-600 ml-2">{formatPct(r.goWildRate)}</span>
                  </div>
                  <div className="mt-1 ml-6 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                    <div className={`h-full rounded-full ${barColor(r.goWildRate)}`}
                      style={{ width: `${Math.min(r.goWildRate, 100)}%` }} />
                  </div>
