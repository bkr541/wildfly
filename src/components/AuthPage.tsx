...
const AuthPage = ({ onSignIn }: AuthPageProps) => {
  ...
  const [signupControls, setSignupControls] = useState<SignupControls>(DEFAULT_SIGNUP_CONTROLS);

  useEffect(() => {
    loadSignupControls().then(setSignupControls).catch(() => {});
  }, []);
        ...
        >
          ...
          >
            ...
            <div className="mt-4">
              ...
              {/* OAuth buttons — shown when relevant providers are enabled for the current mode */}
              {(() => {
                const loginMode = !isSignUp;
                const showGoogle = loginMode ? (signupControls.oauthLoginEnabled && signupControls.oauthLoginGoogle) : (signupControls.oauthSignupEnabled && signupControls.oauthSignupGoogle);
                const showApple  = loginMode ? (signupControls.oauthLoginEnabled && signupControls.oauthLoginApple)  : (signupControls.oauthSignupEnabled && signupControls.oauthSignupApple);
              ...
              {/* Sign-up toggle — only shown when registration is enabled and in login mode */}
              {!isSignUp && signupControls.registrationEnabled && (
      ...
      <AlertDialog open={showLoginError} onOpenChange={setShowLoginError}>
        <AlertDialogContent className="max-w-xs rounded-xl bg-white p-4 pt-10 overflow-visible border border-[#EF4444]">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 h-12 w-12 rounded-full bg-[#FEE2E2] border-2 border-[#EF4444] flex items-center justify-center shadow-sm">
            <HugeiconsIcon icon={AlertCircleIcon} size={22} color="#EF4444" strokeWidth={1.5} />
          </div>
          <AlertDialogHeader className="space-y-1 text-center">
            <AlertDialogTitle className="text-lg font-bold text-[#EF4444] text-center">Incorrect Credentials</AlertDialogTitle>
            <AlertDialogDescription className="text-xs text-[#6B7B7B] text-center">
