...
const FlightDestResults = ({ onBack, responseData }: { onBack: () => void; responseData: string }) => {
  ...
  const { flights, tripType, departureAirport, arrivalAirport } = useMemo(() => {
    try {
      ...
      return { flights: [], tripType: "One Way", departureAirport: "", arrivalAirport: "All" };
    }
  }, [responseData]);
    ...
    <div className="relative flex flex-col min-h-screen bg-[#F2F3F3] overflow-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between px-5 pt-6 pb-2">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-lg font-bold text-[#2E4A4A] tracking-tight uppercase">
            {origin || departureAirport} {`-->`} {arrivalAirport || "All"}
          </h1>
          <span className="text-[11px] text-[#6B7B7B] font-medium">
            {tripType} · {flights.length} flight{flights.length !== 1 ? "s" : ""}
          </span>
        </div>
        <div className="w-10" />
      </header>
      ...
      <div className="flex-1 flex flex-col px-5 pt-1 pb-6 gap-3.5 relative z-10">
        <div className="flex flex-col gap-2.5">
          {groups.map((group) => {
            ...
            // Group flights by hour for timeline
            ...
            const flightsByHour = Object.keys(hourMap)
              .map(Number)
              .sort((a, b) => a - b)
                            ...
                            <div key={h} className="flex gap-2">
                              ...
                              <div className="flex-1 flex flex-col gap-1.5">
                                {items.map(({ flight, idx }) => {
                                  ...
                                  const hasAlert = !!userFlights[flightKey(flight, "alert")];
                                  const hasGoing = !!userFlights[flightKey(flight, "going")];
                                  return (
                                            ...
                                            <div className="flex flex-col">
                                              <span className="text-xs font-bold text-[#2E4A4A]">
                                                ...
                                                {flight.is_plus_one_day && (
                                                  <span className="ml-1 text-[#E89830] text-[9px]">(+1)</span>
                                                )}
                                              </span>
