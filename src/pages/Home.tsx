  ...
  return (
    <div className="flex items-center gap-1.5 w-full">
      {displayChars.map((char, i) => {
        ...
        if (isBlank) {
          return (
            <div key={i} className="relative flex flex-col items-center justify-center rounded-lg border overflow-hidden flex-1 min-w-0"
  ...
  return (
    <div className="flex-shrink-0 w-72 bg-white rounded-2xl border border-[#e3e6e6] shadow-sm p-4 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 flex items-center px-3">
            ...
            <svg width="10" height="10" viewBox="0 0 10 10" fill="none" className="flex-shrink-0">
              <path d="M1 5h8M5 1l4 4-4 4" stroke="#1a2e2e" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm font-semibold text-[#345C5A]">{arrTime} <span className="text-[#6B7B7B] font-normal">{dateLabel}</span></p>
      </div>
    </div>
  );
}
      ...
      <div className="px-6 pb-4 relative z-10">
        ...
        {loading ? (
          <div className="flex gap-3 overflow-x-auto pb-2">
            {[1,2].map(i => (
              <div key={i} className="flex-shrink-0 w-72 h-36 bg-white/60 rounded-2xl border border-[#e3e6e6] animate-pulse" />
            ))}
          </div>
        ) : flights.length === 0 ? (
